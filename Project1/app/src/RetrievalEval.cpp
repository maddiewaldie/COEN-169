		     ...
		     Index *ind)
{
  ...
  // weight = termFrew(dt) * log(totalNumDocs / numDocsContain(termT)) * weight(qt)
  return docTermFreq * log(ind->docCount()/ind->docCount(termID)) * qryTermWeight;
}
		     ...
		     Index *ind)
{
  ...
  // weight = (log(termFreq(dt)) + 1) * log(totalNumDocs/numDocsContain(termT))) * weight(qt)
  return (log(docTermFreq)+1) * log(ind->docCount()/ind->docCount(termID)) * qryTermWeight;
}
		     ...
		     Index *ind)
{
  ...
  // weight = termFreq(dt)/(termFreq(dt) + 0.5 + 1.5(len(D)/avgDocLen)) * log((totalNumDocs - numDocsContain(termT) + 0.5)/(numDocsContain(termT) + 0.5)) * (8 + weight(qt))/(7 + weight(qt))
  double term1 = docTermFreq/(docTermFreq + 0.5 + (1.5 * (ind->docLength(docID)/ind->docLengthAvg())));
  double term2 = log((ind->docCount() - ind->docCount(termID) + 0.5)/(ind->docCount(termID) + 0.5));
  double term3 = (8+qryTermWeight)/(7+qryTermWeight);
