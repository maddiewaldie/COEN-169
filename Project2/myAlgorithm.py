...
for test_user in test_users:
    ...
    for train_user in train_users:
        ...
        euclidean_distance = math.sqrt(sum((train[i] - test[i]) ** 2 for i in range(len(test))))
...
for test_user in test_users:
    ...
    for m in test_movies:
        ...
        numerator = sum(train_ratings[i] / (train_distances[i] + 1e-10) for i in range(len(train_ratings)))
        denominator = sum(1 / (train_distances[i] + 1e-10) for i in range(len(train_distances)))
